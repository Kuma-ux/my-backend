...
/* ==============================
   🆕 HACKED STATUS SCAN
============================== */
app.get("/scan/hacked", async (req, res) => {
    const platform = os.platform();
    ...
    try {
        if (platform === "win32") {
            ...
            // 🔥 Start FULL scan (background)
            try {
                await runCommand(`powershell Start-MpScan -ScanType FullScan`);
                report.scanTriggered = true;
                report.notes.push("Full Defender scan started (this may take a long time)");
            } catch (e) {
                report.notes.push("Failed to start Defender scan (admin required)");
            }

            // 🔍 Read Defender status (non-blocking info)
            try {
                const status = await runCommand(
                    `powershell Get-MpComputerStatus | Select AntivirusEnabled,RealTimeProtectionEnabled,FullScanInProgress`
                );
...
app.listen(4000, () => {
    console.log("HAQ Agent running on http://localhost:4000");
});